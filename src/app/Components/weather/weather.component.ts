...
export class WeatherComponent implements OnInit {
  ...
  getOneCallWeatherByCityName(cityName) {

    this.databaseService.getWeatherByCityName(cityName).subscribe((data: any) => {
      this.cityName = data.name;
      return this.getOneCallWeatherByCoords(data.coord.lat, data.coord.lon);
    });
  ...
  getOneCallWeatherByCoords(lat, lon) {
    ...
    return this.databaseService.getOneCallWeatherByCoords(lat, lon).subscribe((data: {}) => {
      ...
      this.calculateSunPosition(this.currentWeatherData?.sunrise,this.currentWeatherData?.sunset,this.currentWeatherData?.dt);
  ...
  calculateSunPosition(rawSunrise,rawSunset,rawCurTime){
    ...
    let sunPath = document.getElementById("sunPath").getAttribute("path");
    // @ts-ignore
    let sunPathL = sunPath.getTotalLength();

    let svgPathElement: SVGPathElement = new SVGPathElement();
